...
// ÚJ: Ingatlan részleges módosítása (PATCH)
app.patch('/api/ingatlan/:id', (req, res) => {
  const { id } = req.params;
  ...
  if (keys.length === 0) {
    res.status(400).send("Nincs megadva módosítandó adat.");
    return;
  }
  ...
  const sql = `UPDATE ingatlanok SET ${setClause} WHERE id = ?`;
  ...
  db.run(sql, [...values, id], function (err) {
    ...
    res.json({ message: "Ingatlan részlegesen frissítve!", updatedId: id });
  });
});
